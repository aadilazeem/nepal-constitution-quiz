...
# Define SYLLABUS_SECTIONS for your data compatibility
class SYLLABUS_SECTIONS:
    LAW = "Law & Justice"
    ADMIN = "Public Administration"
    CONST = "Constitution"

# FLASHCARD DATA - Just paste your list here
FLASHCARD_DATA = [
...
# MCQ DATA - Just paste your list here
MCQ_DATA = [
    # --- BASIC QUESTIONS ---
...
# CURRENT AFFAIRS DATA - Just paste your list here
...
class DataManager:
    ...
    def _load_flashcards(self) -> List[Question]:
        """Load flashcard questions from FLASHCARD_DATA"""
        return [Question(**item) for item in FLASHCARD_DATA]
    
    def _load_mcqs(self) -> List[MCQQuestion]:
        """Load MCQ questions from MCQ_DATA"""
        return [MCQQuestion(**item) for item in MCQ_DATA]
    
    def _load_news(self) -> List[NewsItem]:
        """Load current affairs news from CURRENT_AFFAIRS_DATA"""
        return [NewsItem(**item) for item in CURRENT_AFFAIRS_DATA]
...
class NavigationButtons(UIComponent):
    """Navigation buttons component"""
    
    def __init__(self, total_items: int, position: str = "top"):
    ...
    def render(self):
        ...
        with col1:
            if st.button("⬅️ Previous", 
                        ...
                        key=f"prev_btn_{self.position}"):
                SessionManager.prev_item()
                st.rerun()
        
        with col2:
            st.markdown(f"<div style='text-align: center; padding: 10px;'><b>Item {st.session_state.current_index + 1} of {self.total_items}</b></div>", 
                       unsafe_allow_html=True)
        
        with col3:
            if st.button("Next ➡️", 
                        disabled=(st.session_state.current_index == self.total_items - 1),
                        use_container_width=True,
                        key=f"next_btn_{self.position}"):
                SessionManager.next_item()
                st.rerun()
...
class Sidebar(UIComponent):
    ...
    def __init__(self):
        self.section_options = ["All", "Law & Justice", "Public Administration", "Constitution"]
        self.difficulty_options = ["All"] + Config.LEVELS
    
    def render(self) -> Dict[str, str]:
        """Render sidebar and return selected filters"""
        st.sidebar.header("⚙️ Practice Filters")
        ...
        section = st.sidebar.selectbox("Section", self.section_options)
        difficulty = st.sidebar.selectbox("Difficulty", self.difficulty_options)
        
        st.sidebar.markdown("---")
        st.sidebar.info("Adjust filters to customize your practice.")
...
class FlashcardView(UIComponent):
    ...
    def render(self):
        ...
        if not st.session_state.show_answer:
            if st.button("💡 Reveal Answer", use_container_width=True, key="reveal_flashcard"):
            ...
            if st.button("🔄 Hide Answer", use_container_width=True, key="hide_flashcard"):
                st.session_state.show_answer = False
                st.rerun()
        
        # Navigation at bottom only
        st.markdown("---")
        NavigationButtons(len(self.questions), position="bottom").render()
...
class MCQView(UIComponent):
    ...
    def _render_question(self, question: MCQQuestion):
        ...
        with col2:
            if st.button("🔄 Clear Answer", key=f"clear_{key}", use_container_width=True):
        ...
        if question.id in st.session_state.user_answers:
            ...
            if selected == question.answer:
                st.success(f"✅ Correct! The answer is **{question.answer}: {correct_text}**")
            else:
                st.error(f"❌ Incorrect. You selected **{selected}**. The correct answer is **{question.answer}: {correct_text}**")
            ...
            for opt_key in sorted(question.options.keys()):
                ...
                if opt_key == question.answer:
                    st.markdown(f"✅ **{opt_key}: {opt_val}** (Correct)")
                elif opt_key == selected:
                    st.markdown(f"❌ **{opt_key}: {opt_val}** (Your answer)")
                else:
...
class CurrentAffairsView(UIComponent):
    ...
    def render(self):
        ...
        # Navigation at bottom only
        st.markdown("---")
        NavigationButtons(len(self.news_items), position="bottom").render()
...
class LoksewaApp:
    ...
    def run(self):
        ...
        st.title("🇳🇵 Loksewa Aayog Learning Hub")
